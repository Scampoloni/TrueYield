...
- Aktueller Stand: alle Core Services ≥ 90% abgedeckt (Gate aktiv und grün). Gesamt-Projektabdeckung inkl. Controller, Modelle und Konfigurationsklassen: siehe Badge oben (lokal zuletzt **93%** Instruction Coverage).
...
|---|---|
| Codeanalyse mit SonarQube | SonarCloud aktiv auf `main`-Branch, Analyse via `sonar-maven-plugin` in CI (non-blocking). Token via Secret `SONAR_TOKEN`. CI-Step: [`.github/workflows/ci.yml`, Zeilen 170–179](../.github/workflows/ci.yml#L170-L179) |
| Komplexes Datenmodell (5 Entitäten) | Portfolio, Holding, Evidence, AuditReport, AuditComment — übererfüllt gegenüber Mindestanforderung (3). Zustandsmodell: [`backend/src/main/java/ch/zhaw/trueyield/model/enums/AuditStatus.java`](../backend/src/main/java/ch/zhaw/trueyield/model/enums/AuditStatus.java) |
| Komplexes Frontend | 3 Rollen mit rollenspezifischen Dashboards, State-Machine-Visualisierung, Sentiment-Badges, Risk-Scores, SVG-Donut-Chart (Asset Allocation), KPI-Karten, SFDR-Ampel, Evidence-Confidence-Badges, Symbol-Autocomplete. SVG-Donut: [`frontend/src/lib/components/DonutChart.svelte`, Zeilen 18–60](../frontend/src/lib/components/DonutChart.svelte#L18-L60) — Route Guards: [`frontend/src/hooks.server.js`, Zeile 22](../frontend/src/hooks.server.js#L22) |
...
| Komplexe Abfragen auf der Datenbank | MongoDB Aggregation Pipeline für Audit-Dashboard (gruppiert nach Status pro Portfolio). [`backend/src/main/java/ch/zhaw/trueyield/repository/AuditReportRepository.java`, Zeilen 23–27](../backend/src/main/java/ch/zhaw/trueyield/repository/AuditReportRepository.java#L23-L27) |
| Komplexe Benutzerverwaltung | 3 RBAC-Rollen (`fund-manager`, `auditor`, `compliance-officer`) mit unterschiedlichen Berechtigungen auf Endpunkt-Ebene (`@PreAuthorize`) und im Frontend (Route Guards). Backend: [`backend/src/main/java/ch/zhaw/trueyield/controller/AuditCommentController.java`, Zeilen 34–44](../backend/src/main/java/ch/zhaw/trueyield/controller/AuditCommentController.java#L34-L44) — Frontend: [`frontend/src/hooks.server.js`, Zeile 22](../frontend/src/hooks.server.js#L22) |
...
| End-to-End Tests (Cypress) | 5 Testdateien, 73 Testfälle: [`frontend/cypress/e2e/auth.cy.js`](../frontend/cypress/e2e/auth.cy.js), [`portfolio.cy.js`](../frontend/cypress/e2e/portfolio.cy.js), [`audit.cy.js`](../frontend/cypress/e2e/audit.cy.js), [`evidence.cy.js`](../frontend/cypress/e2e/evidence.cy.js), [`compliance.cy.js`](../frontend/cypress/e2e/compliance.cy.js) — ausgeführt in CI |
...
**Compliance Officer (Anforderung 23):** Dritte RBAC-Rolle `compliance-officer` mit systemweitem Lesezugriff. Vier dedizierte Endpoints: `GET /api/compliance/overview` (KPI-Übersicht), `/sfdr` (SFDR Article 8/9 Klassifizierung per Portfolio), `/portfolios` (alle Portfolios), `/reports` (alle Audit-Reports). Alle schreibenden Operationen sind blockiert (403 per `@PreAuthorize`). Frontend-Dashboard unter `/compliance` mit drei Tabs (Overview / Portfolios / Audit Reports). JWT-Claim-Extraktion liest den Claim `user_roles` — passend zur Auth0 Post-Login Action, die den Claim ohne Namespace setzt.
...
**Testabdeckung:** JUnit 5 + Mockito für alle Core-Services mit JaCoCo-Gate >= 90 % auf PortfolioService, HoldingService, AuditReportService, AuditCommentService, EvidenceService, UserService und ComplianceService. **480 ausgeführte Testfälle in 35 Testklassen** (lokaler `surefire-reports` Stand; Surefire zählt jede parametrisierte Test-Iteration einzeln) — parametrisierte Tests (`@ParameterizedTest`, `@CsvSource`, `@ValueSource`) und Spring MVC MockMvc-Tests für alle Controller mit rollenbasierter Zugriffsprüfung. Cypress E2E: 5 Testdateien, 73 Testfälle (64 aktiv, 9 skipped).
...
# Preserved academic documentation

> This document preserves the original academic submission material for context. It is historical documentation, not the current portfolio positioning. It may contain dated assumptions, implementation plans, external claims, and terminology that should not be interpreted as a statement of production readiness, regulatory compliance, or current deployment status. See the [portfolio README](../README.md) for the current, restrained project overview.
